@extends('main') @section('title')
    @endsection @section('stylesheet')
    @endsection @section('content')
    <div class="section section-profile bg-light pt-0">
        <div class="container has-sidebar">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">@lang('messages.profile')</li>
            </ol>

            <div class="sidebar">
                <div class="card-info main">
                    <div class="title-bar d-flex" data-bs-toggle="collapse" data-bs-target="#navProfile">
                        <h1 class="h2 text-capitalize text-underline">@lang('messages.profile')</h1>

                        <button class="btn btn-menu" type="button">
                            <img class="icons svg-js" src="{{ asset('img/icons/icon-add-plus.svg') }}" alt="" />
                        </button>
                    </div>

                    <div id="navProfile" class="collapse">
                        <ul class="nav nav-profile">
                            <li>
                                <a href="{{ url(app()->getLocale() . '/profile') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-user.svg') }}" alt="" />
                                    @lang('messages.my_account')
                                </a>
                            </li>
                            <li>
                                <a href="{{ url(app()->getLocale() . '/address') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-map-2.svg') }}" alt="" />
                                    @lang('messages.address')
                                </a>
                            </li>
                            <li>
                                <a href="{{ url(app()->getLocale() . '/tax-invoice') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-map-2.svg') }}" alt="" />
                                    @lang('messages.tax_invoice')
                                </a>
                            </li>
                            <li class="active">
                                <a href="{{ url(app()->getLocale() . '/my-purchase') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-document.svg') }}" alt="" />
                                    @lang('messages.my_purchase')
                                </a>
                            </li>
                            <li>
                                <a href="{{ url(app()->getLocale() . '/my-favourite') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-favorite-2.svg') }}"
                                        alt="" />
                                    @lang('messages.my_favourite')
                                </a>
                            </li>
                            <li>
                                <a href="{{ url(app()->getLocale() . '/reviews') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-star-sharp.svg') }}"
                                        alt="" />
                                    @lang('messages.my_reviews')
                                </a>
                            </li>
                            <li>
                                <a href="{{ url(app()->getLocale() . '/my-coupon') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-ticket-discount.svg') }}"
                                        alt="" />
                                    @lang('messages.my_coupon_point')
                                </a>
                            </li>
                            <li>
                                <a href="{{ url(app()->getLocale() . '/notification') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-bell-2.svg') }}" alt="" />
                                    @lang('messages.notification')
                                </a>
                            </li>
                            <li>
                                <a href="{{ url(app()->getLocale() . '/change-password') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-user.svg') }}" alt="" />
                                    @lang('messages.change_password')
                                </a>
                            </li>
                            <li>
                                <a href="{{ url(app()->getLocale() . '/term-condition') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-note.svg') }}" alt="" />
                                    @lang('messages.term_and_condition')
                                </a>
                            </li>
                            <li>
                                <a href="{{ url(app()->getLocale() . '/privacy-policy') }}">
                                    <img class="icons" src="{{ asset('img/icons/icon-shield-tick.svg') }}"
                                        alt="" />
                                    @lang('messages.privacy_policy')
                                </a>
                            </li>
                            <li>
                                <form id="logout-form" action="{{ url(app()->getLocale() . '/logout') }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                </form>
                                <a href="#"
                                    onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                                    <img class="icons" src="{{ asset('img/icons/icon-logout.svg') }}" alt="" />
                                    @lang('messages.sign_out')
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
                <!--card-info-->
            </div>
            <!--sidebar-->

            <div class="content">
                <ul class="nav nav-buttons">
                    <li><button class="btn active" type="button">All</button></li>
                    <li><button class="btn" type="button">To Pay</button></li>
                    <li><button class="btn" type="button">To Delivery</button></li>
                    <li><button class="btn" type="button">To Receive</button></li>
                    <li><button class="btn" type="button">Completed</button></li>
                    <li><button class="btn" type="button">Cancelled</button></li>
                </ul>
                <div class="card-info purchase pt-2 px-5">
                    <a href="my-purchase-topay.html" class="card-link"></a>
                    <div class="info-row border-bottom-1">
                        <p><strong>Purchase Order No :</strong> PO20240000101</p>

                        <label class="purchase-status toplay">To Pay</label>
                    </div>

                    <ul class="ul-table ul-table-body infos">
                        <li class="photo">
                            <img src="img/thumb/photo-400x455--1.jpg" alt="" />
                        </li>
                        <li class="info">
                            <div class="product-info">
                                <h3>
                                    (+)-1,2-Bis[(2R,5R)-2,5-diethyl
                                    -1-phospholanyl]ethane, 97+%
                                </h3>
                                <label class="label">Size : 5G (Code: ALF-J63245.06 )</label>
                                <p><small>Model : DAE-1217-2304</small></p>
                            </div>
                        </li>
                        <li class="qty">
                            <strong class="fs-16 text-black">x2</strong>
                        </li>
                        <li class="total"><strong>14,338.00฿</strong></li>
                    </ul>

                    <div class="info-row">
                        <p>Total : 3 Item</p>
                        <p class="price">14,338.00฿</p>
                    </div>
                    <div class="info-row">
                        <div class="d-flex gap-2">
                            <img class="icons w-34" src="img/icons/icon-34x34--1.svg" alt="" />

                            <label class="purchase-status">
                                <span class="px-sm-2">Waiting for seller Confirm</span>
                            </label>
                        </div>
                        <p><a class="link link-primary" href="#">See More</a></p>
                    </div>
                </div>
                <!--card-info-->

                <div class="card-info purchase pt-2 px-5">
                    <a href="my-purchase-topay.html" class="card-link"></a>
                    <div class="info-row border-bottom-1">
                        <p><strong>Order No :</strong> SH20240000101</p>

                        <label class="purchase-status toplay">To Pay</label>
                    </div>

                    <ul class="ul-table ul-table-body infos">
                        <li class="photo">
                            <img src="img/thumb/photo-400x455--1.jpg" alt="" />
                        </li>
                        <li class="info">
                            <div class="product-info">
                                <h3>
                                    (+)-1,2-Bis[(2R,5R)-2,5-diethyl
                                    -1-phospholanyl]ethane, 97+%
                                </h3>
                                <label class="label">Size : 5G (Code: ALF-J63245.06 )</label>
                                <p><small>Model : DAE-1217-2304</small></p>
                            </div>
                        </li>
                        <li class="qty">
                            <strong class="fs-16 text-black">x2</strong>
                        </li>
                        <li class="total"><strong>14,338.00฿</strong></li>
                    </ul>

                    <div class="info-row">
                        <p>Total : 3 Item</p>
                        <p class="price">14,338.00฿</p>
                    </div>
                    <div class="info-row">
                        <div class="d-flex gap-3">
                            <img class="icons w-34" src="img/icons/icon-34x34--2.svg" alt="" />

                            <h3 class="fs-14 text-warning">Pay within 14:29 min</h3>
                        </div>
                        <p><a class="link link-primary" href="#">See More</a></p>
                    </div>
                </div>
                <!--card-info-->

                <div class="card-info purchase pt-2 px-5">
                    <a href="my-purchase-delivery.html" class="card-link"></a>
                    <div class="info-row border-bottom-1">
                        <p><strong>Order No :</strong> SH20240000101</p>

                        <label class="purchase-status delivery">To Delivery</label>
                    </div>

                    <ul class="ul-table ul-table-body infos">
                        <li class="photo">
                            <img src="img/thumb/photo-400x455--1.jpg" alt="" />
                        </li>
                        <li class="info">
                            <div class="product-info">
                                <h3>
                                    (+)-1,2-Bis[(2R,5R)-2,5-diethyl
                                    -1-phospholanyl]ethane, 97+%
                                </h3>
                                <label class="label">Size : 5G (Code: ALF-J63245.06 )</label>
                                <p><small>Model : DAE-1217-2304</small></p>
                            </div>
                        </li>
                        <li class="qty">
                            <strong class="fs-16 text-black">x2</strong>
                        </li>
                        <li class="total"><strong>14,338.00฿</strong></li>
                    </ul>

                    <div class="info-row">
                        <p>Total : 3 Item</p>
                        <p class="price">14,338.00฿</p>
                    </div>
                    <div class="info-row">
                        <p>
                            We will deliver the product to you<br />
                            within 3-5 days.
                        </p>
                        <p><a class="link link-primary" href="#">See More</a></p>
                    </div>
                </div>
                <!--card-info-->

                <div class="card-info purchase pt-2 px-5">
                    <a href="my-purchase-receive.html" class="card-link"></a>
                    <div class="info-row border-bottom-1">
                        <p><strong>Order No :</strong> SH20240000101</p>

                        <label class="purchase-status receive">To Receive</label>
                    </div>

                    <ul class="ul-table ul-table-body infos">
                        <li class="photo">
                            <img src="img/thumb/photo-400x455--1.jpg" alt="" />
                        </li>
                        <li class="info">
                            <div class="product-info">
                                <h3>
                                    (+)-1,2-Bis[(2R,5R)-2,5-diethyl
                                    -1-phospholanyl]ethane, 97+%
                                </h3>
                                <label class="label">Size : 5G (Code: ALF-J63245.06 )</label>
                                <p><small>Model : DAE-1217-2304</small></p>
                            </div>
                        </li>
                        <li class="qty">
                            <strong class="fs-16 text-black">x2</strong>
                        </li>
                        <li class="total"><strong>14,338.00฿</strong></li>
                    </ul>

                    <div class="info-row">
                        <p>Total : 3 Item</p>
                        <p class="price">14,338.00฿</p>
                    </div>
                    <div class="info-row">
                        <p>The product is being shipped.</p>
                        <p><a class="link link-primary" href="#">See More</a></p>
                    </div>
                </div>
                <!--card-info-->

                <div class="card-info purchase pt-2 px-5">
                    <a href="my-purchase-completed-receive.html" class="card-link"></a>
                    <div class="info-row border-bottom-1">
                        <p><strong>Order No :</strong> SH20240000101</p>

                        <label class="purchase-status completed">Completed</label>
                    </div>

                    <ul class="ul-table ul-table-body infos">
                        <li class="photo">
                            <img src="img/thumb/photo-400x455--1.jpg" alt="" />
                        </li>
                        <li class="info">
                            <div class="product-info">
                                <h3>
                                    (+)-1,2-Bis[(2R,5R)-2,5-diethyl
                                    -1-phospholanyl]ethane, 97+%
                                </h3>
                                <label class="label">Size : 5G (Code: ALF-J63245.06 )</label>
                                <p><small>Model : DAE-1217-2304</small></p>
                            </div>
                        </li>
                        <li class="qty">
                            <strong class="fs-16 text-black">x2</strong>
                        </li>
                        <li class="total"><strong>14,338.00฿</strong></li>
                    </ul>

                    <div class="info-row">
                        <p>Total : 3 Item</p>
                        <p class="price">14,338.00฿</p>
                    </div>
                    <div class="info-row">
                        <p class="text-success">
                            The product has been shipped.<br />
                            Please press to receive the product.
                        </p>
                        <p><a class="link link-primary" href="#">See More</a></p>
                    </div>
                </div>
                <!--card-info-->

                <div class="card-info purchase pt-2 px-5">
                    <a href="my-purchase-completed-.html" class="card-link"></a>
                    <div class="info-row border-bottom-1">
                        <p><strong>Order No :</strong> SH20240000101</p>

                        <label class="purchase-status completed">Completed</label>
                    </div>

                    <ul class="ul-table ul-table-body infos">
                        <li class="photo">
                            <img src="img/thumb/photo-400x455--1.jpg" alt="" />
                        </li>
                        <li class="info">
                            <div class="product-info">
                                <h3>
                                    (+)-1,2-Bis[(2R,5R)-2,5-diethyl
                                    -1-phospholanyl]ethane, 97+%
                                </h3>
                                <label class="label">Size : 5G (Code: ALF-J63245.06 )</label>
                                <p><small>Model : DAE-1217-2304</small></p>
                            </div>
                        </li>
                        <li class="qty">
                            <strong class="fs-16 text-black">x2</strong>
                        </li>
                        <li class="total"><strong>14,338.00฿</strong></li>
                    </ul>

                    <div class="info-row">
                        <p>Total : 3 Item</p>
                        <p class="price">14,338.00฿</p>
                    </div>
                    <div class="info-row">
                        <p class="text-success">Your product has been shipped.</p>
                        <p><a class="link link-primary" href="#">See More</a></p>
                    </div>
                </div>
                <!--card-info-->

                <div class="card-info purchase pt-2 px-5">
                    <a href="my-purchase-cancel.html" class="card-link"></a>
                    <div class="info-row border-bottom-1">
                        <p><strong>Order No :</strong> SH20240000101</p>

                        <label class="purchase-status cancel">Cancel</label>
                    </div>

                    <ul class="ul-table ul-table-body infos">
                        <li class="photo">
                            <img src="img/thumb/photo-400x455--1.jpg" alt="" />
                        </li>
                        <li class="info">
                            <div class="product-info">
                                <h3>
                                    (+)-1,2-Bis[(2R,5R)-2,5-diethyl
                                    -1-phospholanyl]ethane, 97+%
                                </h3>
                                <label class="label">Size : 5G (Code: ALF-J63245.06 )</label>
                                <p><small>Model : DAE-1217-2304</small></p>
                            </div>
                        </li>
                        <li class="qty">
                            <strong class="fs-16 text-black">x2</strong>
                        </li>
                        <li class="total"><strong>14,338.00฿</strong></li>
                    </ul>

                    <div class="info-row">
                        <p>Total : 3 Item</p>
                        <p class="price">14,338.00฿</p>
                    </div>
                    <div class="info-row">
                        <p class="text-danger">You cancel the order</p>
                        <p><a class="link link-primary" href="#">See More</a></p>
                    </div>
                </div>
                <!--card-info-->

                <div class="card-info purchase pt-2 px-5">
                    <a href="return-refund.html" class="card-link"></a>
                    <div class="info-row border-bottom-1">
                        <p><strong>Order No :</strong> SH20240000101</p>

                        <label class="purchase-status return">Return/Refund</label>
                    </div>

                    <ul class="ul-table ul-table-body infos">
                        <li class="photo">
                            <img src="img/thumb/photo-400x455--1.jpg" alt="" />
                        </li>
                        <li class="info">
                            <div class="product-info">
                                <h3>
                                    (+)-1,2-Bis[(2R,5R)-2,5-diethyl
                                    -1-phospholanyl]ethane, 97+%
                                </h3>
                                <label class="label">Size : 5G (Code: ALF-J63245.06 )</label>
                                <p><small>Model : DAE-1217-2304</small></p>
                            </div>
                        </li>
                        <li class="qty">
                            <strong class="fs-16 text-black">x2</strong>
                        </li>
                        <li class="total"><strong>14,338.00฿</strong></li>
                    </ul>

                    <div class="info-row">
                        <p>Total : 3 Item</p>
                        <p class="price">14,338.00฿</p>
                    </div>
                    <div class="info-row">
                        <p class="text-orange">
                            Waiting for return approval By : admin
                        </p>
                        <p><a class="link link-primary" href="#">See More</a></p>
                    </div>
                </div>
                <!--card-info-->
            </div>
            <!--content-->
        </div>
        <!--container-->
    </div>
    <!--section-->
    @endsection @section('script')
@endsection
