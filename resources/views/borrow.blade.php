                        ...
                        @if (count($cart) > 0)
                            @foreach ($cart as $id => $item)
                                @php
                                    ...
                                    $remainingStock = max($available - $borrowed, 0);
                                @endphp
                                <div class="equipment-item mb-3 mt-4" data-id="{{ $id }}"
                                    data-available="{{ $available }}" data-borrowed="{{ $borrowed }}">
                                    ...
                                    <div class="equipment-details">
                                        <h5>{{ $item['name'] }}</h5>
                                        <p class="quantity">จำนวน: <span class="quantity-value">{{ $borrowed }}</span>
                                            ชิ้น</p>
                                        <div class="stock-info">
                                            <small>คงเหลือในสต็อก: {{ $remainingStock }} ชิ้น</small>
                                        </div>
                                    </div>
                                    <div class="action-buttons">
                                        <button type="button" class="btn btn-success btn-sm increase"
                                            data-id="{{ $id }}"
                                            {{ $remainingStock <= 0 ? 'disabled' : '' }}>+</button>
                                        <button type="button" class="btn btn-warning btn-sm decrease"
                                            data-id="{{ $id }}"
                                            {{ $borrowed <= 1 ? 'disabled' : '' }}>-</button>
                                        <button type="button" class="btn btn-danger btn-sm remove"
                                            data-id="{{ $id }}">ลบ</button>
                                    </div>
