...
<form id="myForm" action="{{ route('administrator.promotion.submit') }}" method="POST" enctype="multipart/form-data">
    @csrf
                    ...
                    @foreach ($language as $index => $languages)
                        <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="tab-{{ $languages->code }}" role="tabpanel">
                            <div class="mb-4 row">
                                ...
                                <div class="col-md-10">
                                    <input class="form-control" type="text" placeholder="Input Address" id="address" name="address[{{ $languages->id }}]"/>
                                </div>
                            </div>
...
<script>
    $(document).ready(function() {
        $("#image").fileinput({
            uploadUrl: "{{ route('administrator.promotion.submit') }}",
            showRemove: false,
