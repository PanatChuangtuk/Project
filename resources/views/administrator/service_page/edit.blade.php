...
@section('content') 
<form id="myForm" action="{{ route('administrator.service.update',$service->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="tab-{{ $languages->code }}" role="tabpanel">
                    <div class="mb-4 row">
                        ...
                        <div class="col-md-10">
                            <input class="form-control" type="text" placeholder="Input Name" id="name" name="name[{{ $languages->id}}]" value="{{ $serviceContent[$language->id]->name ?? '' }}"/>
                        </div>
                    </div>
                    ...
                    <div class="mb-4 row">
                        ...
                        <div class="col-md-10">
                            <textarea name="description[{{ $languages->id }}]" class="areaEditor form-control" cols="30" rows="5" value="{{ $serviceContent[$language->id]->description ?? '' }}" id="description-{{ $languages->id }}"></textarea>
                        </div>
                    </div>
                </div>
                @endforeach  
...
<script>
    $(document).ready(function() {
        $("#image").fileinput({
            uploadUrl: "{{ route('administrator.service.update', $service->id) }}",
            ...
            initialPreview: [
                @if($service)
                src="{{ asset('storage/file/service/' . basename($service->image)) }}"
                @else
                null
                @endif
                ], initialPreviewConfig: [
                @if($service)
                {caption: "{{basename($service->image)}}", key: 1} 
                 @else
