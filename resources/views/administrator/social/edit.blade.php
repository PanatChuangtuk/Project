...
@section('content') 
<form id="myForm" action="{{ route('administrator.banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data" class="container">
    @csrf
    ...
    <div class="card p-4">
        ...
        <div class="mb-4 row">
            ...
            <div class="col-md-10">
                <textarea  class="form-control" id="url" name="url" placeholder="Enter URL"  value="{{$banner->url}}"></textarea>
            </div>
        </div>
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-10 d-flex align-items-center">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault" value="1" name="status" {{ $banner->status == 1 ? 'checked' : '0' }} />
...
<script>
    $(document).ready(function() {
        $("#image").fileinput({
            uploadUrl: "{{ route('administrator.banner.update', $banner->id) }}",
            ...
            initialPreview: [
                @if($banner->image)
                src="{{ asset('storage/file/banner/' . basename($banner->image)) }}"
                @else
                null
                @endif
            ],
            initialPreviewConfig: [
                @if($banner)
                {caption: "{{ basename($banner->image) }}", key: 1} 
                @else
