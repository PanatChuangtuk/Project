...
@section('content') 
<form id="myForm" action="{{ route('administrator.update', $news->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
                    ...
                    @foreach ($languages as $index => $language)
                        <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="tab-{{ $language->code }}" role="tabpanel">
                            <div class="mb-4 row">
                                ...
                                <div class="col-md-10">
                                    <input class="form-control" type="text" value="{{ $newsContents[$language->id]->name ?? '' }}" id="name-{{ $language->id }}" name="name[{{ $language->id }}]" />
                                </div>
                            </div>
                            <div class="mb-4 row">
...
<script>
    $(document).ready(function() {
        $("#image").fileinput({
            uploadUrl: "{{ route('administrator.update', $news->id) }}",
            deleteUrl: "{{ route('administrator.delete.image', $news->id).'?_token='.csrf_token() }}",
