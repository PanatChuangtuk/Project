...
@section('content') 
<form id="myForm" action="{{ route('administrator.milestone.submit') }}" method="POST" enctype="multipart/form-data">
    @csrf
                    ...
                    @foreach ($language as $index => $languages)
                        <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="tab-{{ $languages->code }}" role="tabpanel">
                            <div class="mb-4 row">
                                ...
                                <div class="col-md-10">
                                    <input class="form-control" type="text" placeholder="Input Name" id="name-{{ $languages->id }}" name="name[{{ $languages->id }}]"/>
                                </div>
                            </div>
                            <div class="mb-4 row" id="description-field-{{ $languages->id }}">
                                <label for="description-{{ $languages->id }}" class="col-md-2 col-form-label">Description</label>
                                <div class="col-md-10">
                                    <textarea class="form-control" style="height:221px" id="description-{{ $languages->id }}" name="description[{{ $languages->id }}]"></textarea>
                                </div>
                            </div>

                            <div class="mb-4 row content-field" id="contentFiled-{{ $languages->id }}">
                                <label for="content-{{ $languages->id }}" class="col-md-2 col-form-label">Content</label>
                                <div class="col-md-10">
                                    <textarea name="content[{{ $languages->id }}]" class="areaEditor form-control" cols="30" rows="5" id="content-{{ $languages->id }}"></textarea>
                                </div>
                            </div>
                        </div>
                    @endforeach  
...
<script>
    $(document).ready(function() {
        $("#icon").fileinput({
            uploadUrl: "{{ route('administrator.milestone.submit') }}",
            showRemove: false,
