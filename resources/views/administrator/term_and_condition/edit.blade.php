...
@section('content') 
<form id="myForm" action="{{ route('administrator.term_and_condition.update', $news->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
                ...
                <div class="tab-content">
                    @foreach ($languages as $index => $language)
                        <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="tab-{{ $language->code }}" role="tabpanel">
                            <div class="mb-4 row">
                                <label for="name-{{ $language->id }}" class="col-md-2 col-form-label">Title</label>
                                <div class="col-md-10">
                                    <input class="form-control" type="text" value="{{ $newsContents[$language->id]->title ?? '' }}" id="title-{{ $language->id }}" name="title[{{ $language->id }}]" />
                                </div>
                            </div>
                            {{-- <div class="mb-4 row">
                            ...
                            <div class="mb-4 row">
                                ...
                                <div class="col-md-10">
                                    <textarea name="content[{{ $language->id }}]" class="areaEditor form-control" cols="30" rows="5" id="content-{{ $language->id }}">{{ $newsContents[$language->id]->content ?? '' }}</textarea>
                                </div>
                            </div>
                        </div>
                    @endforeach
                    ...
                    {{-- <div class="tab-pane fade" id="tab-settings" role="tabpanel">
                        <div class="mb-4 row">
                            ...
                            <div class="col-md-10">
                                <input class="form-control" type="text" placeholder="Input Slug" value="{{$news->slug}}" id="slug" name="slug" />
                            </div>
                        </div>
                        ...
                        <div class="form-check form-switch mt-4">
                            ...
                            <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault" value="1" name="status" {{ $news->status == 1 ? 'checked' : '' }} />
...
<script>
    $(document).ready(function() {
        $("#image").fileinput({
            uploadUrl: "{{ route('administrator.term_and_condition.update', $news->id) }}",
            deleteUrl: "{{ route('administrator.term_and_condition.delete.image', $news->id).'?_token='.csrf_token() }}",
            ...
            initialPreview: [
                @if($newsImage)
                    src="{{ asset('storage/file/news/' . basename($newsImage->image)) }}"
                @else
                    null
                @endif
            ],
            initialPreviewConfig: [
                @if($newsImage)
                    { caption: "{{ basename($newsImage->image) }}", key: 1 }
                @else
